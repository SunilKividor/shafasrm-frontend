...
class LoginRepo with BaseApiRepository{
  ...
  Future<Either<DioException,AuthResModel>> login(LoginReqModel loginReqModel) async {
...
class SignUpRepo with BaseApiRepository{
  ...
  Future<Either<DioException,AuthResModel>> signup(SignupReqModel loginReqModel) async {
    final response = getApiResponse<AuthResModel>(
      request: () => signupClient.signup(loginReqModel,'application/json')
      );
      print("Inside login repo");
      return response;
  }
